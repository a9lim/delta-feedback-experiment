"""Short fixed-config CCE sweep, for the devices whose head runs CCE (off Hopper).
    ...
    python scripts/cce_bench.py --dim 768 --rows 2 --candidates bf16-base,bf16-d64
...
Hopper trains the dense cuBLAS head instead (delta_feedback_experiment/head.py;
the replay bench times it in place). An FP8 CCE recipe ran 4-7x slower than
BF16 on the H100 and lost on Ada, and was removed.
...
CANDIDATES = {
    "bf16-base": Tile(128, 128, 32, 4, 4),
    "bf16-d64": Tile(128, 128, 64, 4, 3),
    "bf16-v256": Tile(128, 256, 64, 8, 3),
    "bf16-b256": Tile(256, 128, 64, 8, 3),
}
"""Fixed configurations; both halves share one, so the logits agree."""


def select(tile: Tile) -> None:
    ...
    ta._cce_best_config = tile.config
    ta._cce_best_config_hopper = tile.config
...
def main() -> None:
    ...
    parser.add_argument("--candidates", default="bf16-base,bf16-d64,bf16-b256")
    ...
    def run(e, y, filter_eps):
        sink.zero_()
        loss, grad = call(e, y, filter_eps)
    ...
    production_reference = run(embeddings, targets, filter_eps)
    # Explicitly exercise accumulated, nonzero sink semantics outside timing.
    loss, de = call(embeddings, targets, filter_eps)
    ...
    result = {
        ...
        "scope": "Captured CCE fwd+bwd and vocabulary ordering; excludes the per-step sink zero. Initialized synthetic normalized readouts, not a trained checkpoint.",
        "results": [],
    }
    ...
    save()
    for name in names:
        tile = CANDIDATES[name]
        select(tile)
        entry = {"name": name, **asdict(tile)}
        started = time.monotonic()
        try:
            dense_check = compare(run(check_e, check_y, None), dense)
            full = run(embeddings, targets, filter_eps)
            ...
            if not good:
                ...
                with torch.cuda.stream(stream):
                    for _ in range(2):
                        sink.zero_()
                        call(embeddings, targets, filter_eps)
                ...
                try:
                    with torch.cuda.graph(graph):
                        graph_loss, graph_grad = call(embeddings, targets, filter_eps)
                finally:
                    if gc_enabled:
                        gc.enable()
                ...
                # candidate's ordinary call, not a different tile.
