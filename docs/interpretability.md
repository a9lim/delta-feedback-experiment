...
| [`delta eval`](operations.md#downstream-evaluation) | Workspace tasks, zero- or few-shot (`--shots`), with Standard, Soft, or Fused teacher-forced continuation scoring; the queue runs it after a finished schedule. `--baseline` adds a paired comparison against a published model. |
