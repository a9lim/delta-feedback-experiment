...
\node[note,anchor=east,text width=2.5cm] at (7.95,7.25)
  {Pass 1 / plain prefix:\\embedding entry};
...
% The outer feedback path carries p_t to fusion with E(x_{t+1}).
...
\node[box,fill=yellow,text width=3.3cm] (fusiontoken) at (15.2,4.62)
  {Token RMSNorm $N_e$};
\draw[flow] (nextembedding) -- (fusiontoken);
\node[box,fill=purple,text width=7.5cm,minimum height=1.12cm]
  (fusion) at (17.7,6.35)
  {\textbf{Shared concat-linear fusion}\quad $2D\to D$\\
   $u_t=W_f\,[N_e(e_{t+1});\,p_t+\xi_t]$};
\draw[flow,rounded corners=3pt] (fusiontoken.north) -- (15.2,5.5)
  -| ($(fusion.south)+(-1.5,0)$);
