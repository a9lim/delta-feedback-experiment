...
\node[box,fill=yellow,text width=4.6cm] (tokennorm) at (8.2,6.6)
  {\textbf{Shared token RMSNorm $N_e$}\\$\bar e_t=N_e(e_t)$};
\node[box,fill=purple!50,text width=4.6cm] (seed) at (8.2,9.7)
  {\textbf{Column seed $s_t$}\\plain $\bar e_t$ or incoming fusion};
\draw[flow] (tokens) -- (embedding);
\draw[flow] (embedding) -- (tokennorm);
\draw[flow] (tokennorm) -- (seed);
...
% The outer feedback path carries p_t to fusion with N_e(E(x_{t+1})).
% Both token paths share E and N_e; the whole row is normalized once.
...
\node[box,fill=yellow,text width=3.3cm] (nexttokennorm) at (15.2,4.62)
  {\textbf{Shared RMSNorm}\\$\bar e_{t+1}=N_e(e_{t+1})$};
\draw[flow] (nextembedding) -- (nexttokennorm);
\node[box,fill=purple,text width=7.5cm,minimum height=1.12cm]
  (fusion) at (17.7,6.35)
  {\textbf{Shared concat-linear fusion}\quad $2D\to D$\\
   $u_t=W_f\,[\bar e_{t+1};\,p_t+\xi_t]$};
\draw[flow,rounded corners=3pt] (nexttokennorm.north) -- (15.2,5.5)
  -| ($(fusion.south)+(-1.5,0)$);
...
\node[note,text width=5.8cm] at (16.9,2.05)
  {One payload RMSNorm, then shared jitter.\\Evaluation: $\xi_t=0$.};
