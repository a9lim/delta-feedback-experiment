...
def test_condition_letters_name_feedback_and_loop():
    for text, canonical in (("n", "n"), ("f", "f"), ("vf", "fv"), ("v", "v")):
    ...
    for text in ("nf", "fn", "nv", "nn"):
        with pytest.raises(ValueError, match="not n alone"):
            parse_condition(text)
    assert condition_config("v").loop and condition_config("fv").loop
    assert not condition_config("f").loop
    for text in ("", "x", "ff", "l", "fl"):
...
def test_payload_and_auxiliary_initialization_pair_across_all_conditions():
    ...
    # ``v`` re-enters with the token itself and ``n`` never re-enters, so
    # every condition has ``f``'s parameter set.
    assert all(state.keys() == states[0].keys() for state in states)
    common = set(states[0])
...
def test_routing_sub_tiles_cover_the_head_width():
    ...
    for num_heads in (2, 3, 4, 6):
        ...
        # Off Hopper the backward keeps the forward's warp count.
        assert backward_warps == forward_warps
    for num_heads, head_dim in ((4, 16), (2, 32), (2, 96), (4, 192), (2, 1024)):
        _, block_k, tiles, *_ = _route_launch(num_heads, head_dim, cpu)
...
def test_looped_columns_reenter_through_the_shared_fusion():
    ...
    column's jittered payload with the position's own token; every column is
    read out; a column never depends on the columns after it; and the first
    column is the single-column model."""
    model = tiny("fv")
    toks = tokens()
    ...
    assert combine_column_losses([[one, two]]).item() == 3.0  # v alone
...
def test_loop_once_pairs_with_flat_values_and_gradients():
    geometry = {"layers": 16}
    flat, loop = tiny("f", **geometry).train(), tiny("fv", **geometry).train()
    losses = []
...
def test_checkpointing_preserves_feedback_loop_and_auxiliary_gradients():
    models = [tiny("fv").train(), tiny("fv").train()]
...
@torch.no_grad()
@pytest.mark.parametrize("condition", ["fv", "v"])
...
def test_partial_checkpointing_counts_blocks_and_preserves_gradients():
    ...
    numerically."""
    raw, partial = tiny("fv").train(), tiny("fv").train()
...
def test_unit_scale_entry_keeps_reentry_contracting():
    ...
    model = tiny(
        "fv", dim=128, head_dim=64, pkda_head_dim=64, layers=8
    ).train()
